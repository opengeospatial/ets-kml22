...
/**
 * Unmarshals KML 2.x geometry representations (except <code>&lt;Model&gt;</code>) from
...
public class KmlGeometryUnmarshaller {
	 ...
	 * @param namespaceURI The name of the target namespace (an absolute URI value).
	 */
	...
	/**
	 * Creates a JTS geometry object from a KML 2.x geometry representation. The provided
	 * node must be either an Element or a Document node; if the latter, the geometry must
	 * be the document element.
	 * @param node a DOM Document or Element node representation of a KML 2 geometry
	 * element.
	...
	/**
	 * Creates a Point geometry object from a source KML2 Point representation. The ID of
	 * the spatial reference system is set to 0 (undefined).
	 * @param geoElem A DOM Element representing a kml:Point element.
	...
	/**
	 * Creates a LineString object from a KML2 LineString representation. The ID of the
	 * spatial reference system is set to 0 (undefined).
	 * @param geoElem a DOM Element representing a gml:LineString element.
	 ...
	 * @param geoElem a DOM Element representing a kml:LinearRing element.
	...
	/**
	 * Creates a Polygon geometry object from a source KML2 Polygon representation. The ID
	 * of the spatial reference system is set to 0 (undefined).
	 * @param geoElem a DOM Element representing a kml:Polygon element.
	...
	Polygon unmarshalPolygon(Element geoElem) {
		...
		Polygon poly = geoFactory.createPolygon(rings.remove(0), rings.toArray(new LinearRing[nRings - 1]));
		return poly;
	}

	/**
	 * Creates a GeometryCollection geometry object from a source KML2 MultiGeometry
	 * representation with one or more geometry members.
	 * @param geoElem a DOM Element representing a kml:MultiGeometry element.
	...
	GeometryCollection unmarshalMultiGeometry(Element geoElem) {
		...
		GeometryCollection geomColl = geoFactory.createGeometryCollection(geometries.toArray(new Geometry[nMembers]));
		return geomColl;
	}

	/**
	 * Builds a raw coordinate sequence from the tuples included in a given coordinates
	 * element. According to the KML reference, coordinates must be specified as a
	 * space-separated list of 2D or 3D tuples: lon,lat[,alt]. An
	 * <code>IllegalArgumentException</code> is thrown if a coordinate tuple is is not 2D
	 * or 3D.
	 * @param coords a DOM Node representing a kml:coordinates element
	 * @return a CoordinateArraySequence that can be used to create primitive geometry
	 * objects
	 */
	CoordinateArraySequence buildCoordinateArraySequence(Node coords) {
		...
		for (int i = 0; i < nTuples; i++) {
			...
			if (crsDim == 2) {
				coord = new Coordinate(Double.parseDouble(values[0]), Double.parseDouble(values[1]));
			}
			else {
				coord = new Coordinate(Double.parseDouble(values[0]), Double.parseDouble(values[1]),
