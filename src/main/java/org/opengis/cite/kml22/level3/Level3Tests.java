...
public class Level3Tests {
	...
	/**
	 * Checks if conformance level 3 tests will be run. These tests will be run only if
	 * the test run argument {@link org.opengis.cite.kml22.TestRunArg#ICS ics} has a value
	 * &gt; 2; otherwise the tests are skipped.
	...
	public void runConformanceLevel3(ITestContext testContext) {
		...
		if ((null != obj)) {
			...
			Assert.assertTrue(level.intValue() > 2, "Conformance level 3 will not be checked since ics = " + level);
		}
	}
