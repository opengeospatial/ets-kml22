...
/**
 * <p>Contains test methods that apply to view elements (kml:Camera, kml:LookAt).</p>
 * 
...
public class ViewTests extends BaseFixture {
    ...
    /**
     * [{@code Test}] Verifies that a kml:LookAt element satisfies all of the
     * following constraints:
     * 
     * <ol>
     * <li>if it is not a descendant of kml:Update, it contains all of the
     * following child elements: kml:longitude, kml:latitude, and kml:range;</li>
     * <li>0 &lt;= kml:tilt &lt;= 90;</li>
    ...
    public void verifyLookAt() {
        ...
        try {
            lookAtNodes = XMLUtils.evaluateXPath(this.testSubject,
                    "//kml:LookAt[not(ancestor::kml:Update)]", NS_MAP);
        ...
        for (int i = 0; i < lookAtNodes.getLength(); i++) {
            ...
            try {
                ETSAssert.assertXPath(
                        "kml:longitude and kml:latitude and kml:range", lookAt,
                        NS_MAP);
            } catch (AssertionError e) {
                errHandler.addError(ErrorSeverity.ERROR,
                        ErrorMessage.format("level1.LookAt.err1"),
                        new ErrorLocator(-1, -1, XMLUtils.getXPointer(lookAt)));
            }
            try {
                ETSAssert
                        .assertXPath(
                                "(string(kml:altitudeMode) != 'clampToGround') and kml:altitude",
                                lookAt, NS_MAP);
            } catch (AssertionError e) {
                errHandler.addError(ErrorSeverity.ERROR,
                        ErrorMessage.format("level1.LookAt.err4"),
                        new ErrorLocator(-1, -1, XMLUtils.getXPointer(lookAt)));
            }
     ...
     * @param errHandler
     *            The error handler that accepts a reported constraint
     *            violation.
     */
    void checkTiltRange(Element lookAt, ValidationErrorHandler errHandler) {
        Node tiltNode = lookAt.getElementsByTagNameNS(Namespaces.KML22, "tilt")
                .item(0);
        if (null != tiltNode) {
            double tilt = Double.parseDouble(tiltNode.getTextContent());
            if (tilt < 0 || tilt > 90) {
                errHandler.addError(ErrorSeverity.ERROR,
                        ErrorMessage.format("level1.LookAt.err3"),
                        new ErrorLocator(-1, -1, XMLUtils.getXPointer(lookAt)));
            }
        }
    }
