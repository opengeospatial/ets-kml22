...
public class VerifyXMLUtils {
    ...
    public void getXPointerToEntryId() throws SAXException, IOException {
        Document doc = docBuilder.parse(this.getClass().getResourceAsStream(
                "/atom-feed.xml"));
        Element entry = (Element) doc.getElementsByTagNameNS(ATOM_NS, "entry")
                .item(0);
        Element id = (Element) entry.getElementsByTagNameNS(ATOM_NS, "id")
                .item(0);
        String xptr = XMLUtils.getXPointer(id);
        assertTrue("Expected pointer to start with namespace binding context.",
                xptr.startsWith("xmlns("));
        assertTrue("Expected pointer to contain 'xpointer((//tns:id)[2])'.",
                xptr.contains("xpointer((//tns:id)[2])"));
    }
}
