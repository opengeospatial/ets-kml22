...
/**
 * Verifies the behavior of the test class KmlDocumentTests. Test stubs replace fixture
...
public class VerifyKmlDocumentTests {
	...
	public void testNonKMLDocIsInvalid() throws SAXException, IOException {
		...
		when(suite.getAttribute(SuiteAttribute.KML_SCHEMA.getName())).thenReturn(kmlSchema);
		KmlDocumentTests iut = new KmlDocumentTests();
	...
	@Test
	public void emptyObjectWithoutIdShouldFail() throws SAXException, IOException {
		thrown.expect(AssertionError.class);
		thrown.expectMessage("is empty and does not have an @id attribute");
		Document doc = docBuilder.parse(this.getClass().getResourceAsStream("/kml/emptyPlacemarkWithoutId.xml"));
		when(suite.getAttribute(SUBJ)).thenReturn(doc);
		when(suite.getAttribute(SuiteAttribute.KML_SCHEMA.getName())).thenReturn(kmlSchema);
		KmlDocumentTests iut = new KmlDocumentTests();
		iut.setTestSubject(doc);
