...
public class VerifySpatialTests {
	...
	@Test
	public void checkLatRange_SouthGreaterThanNorth() throws SAXException, IOException {
		Document doc = docBuilder.parse(this.getClass().getResourceAsStream("/kml/spatial/LatLonAltBox-SgtN.xml"));
		...
		assertTrue("Unexpected error message.",
				errHandler.toString().contains("kml:north must be greater than kml:south"));
	}
	...
	public void lineStringHasTruncatedTuple() throws SAXException, IOException {
		...
		Document doc = docBuilder
			.parse(this.getClass().getResourceAsStream("/kml/spatial/Placemark-TruncatedLineString.xml"));
		when(suite.getAttribute(SUBJ)).thenReturn(doc);
		SpatialTests iut = new SpatialTests();
		iut.setTestSubject(doc);
	...
	public void invalidPolygonBoundary() throws SAXException, IOException {
		...
		Document doc = docBuilder.parse(this.getClass().getResourceAsStream("/kml/spatial/invalidPolygonBoundary.xml"));
		when(suite.getAttribute(SUBJ)).thenReturn(doc);
		SpatialTests iut = new SpatialTests();
		iut.setTestSubject(doc);
	...
	public void resourceMapIsValid() throws SAXException, IOException {
		URL url = this.getClass().getResource("/kml/models/Model-ResourceMap.xml");
	...
	public void resourceMapIsInvalid() throws SAXException, IOException {
		URL url = this.getClass().getResource("/kml/models/Model-ResourceMap-Invalid.xml");
		Document doc = docBuilder.parse(url.toString());
		SpatialTests iut = new SpatialTests();
		iut.setTestSubject(doc);
		Element mapping = (Element) doc.getDocumentElement()
			...
			.item(0);
		iut.checkModelResourceMap(mapping, this.errHandler);
		assertEquals("Unexpected number of errors.", 2, this.errHandler.getErrorCount());
	}
	...
	public void textureAliasHasMissingTarget() throws SAXException, IOException {
		...
		URL url = this.getClass().getResource("/kml/models/Model-AliasTargetMissing.xml");
		Document doc = docBuilder.parse(url.toString());
		SpatialTests iut = new SpatialTests();
		iut.setTestSubject(doc);
	...
	@Test
	public void tessellatedLineStringHasModeClampToGround() throws SAXException, IOException {
		URL url = this.getClass().getResource("/kml/spatial/LineString-ClampToGround.xml");
		Document doc = docBuilder.parse(url.toString());
		SpatialTests iut = new SpatialTests();
		iut.setTestSubject(doc);
	...
	@Test
	public void tessellatedLineStringWithoutAltitudeMode() throws SAXException, IOException {
		URL url = this.getClass().getResource("/kml/spatial/LineString-Tessellate.xml");
		Document doc = docBuilder.parse(url.toString());
		SpatialTests iut = new SpatialTests();
		iut.setTestSubject(doc);
		iut.verifyGeometryTesselate();
	...
	@Test
	public void tessellatedPolygonHasModeAbsolute() throws SAXException, IOException {
		thrown.expect(AssertionError.class);
		thrown.expectMessage("has kml:altitudeMode not equal to \"clampToGround\"");
		URL url = this.getClass().getResource("/kml/spatial/Polygon-Absolute.xml");
		Document doc = docBuilder.parse(url.toString());
		SpatialTests iut = new SpatialTests();
		iut.setTestSubject(doc);
		iut.verifyGeometryTesselate();
	...
	public void textureAliasInModel() throws SAXException, IOException {
		URL url = this.getClass().getResource("/kml/models/model_relative_target.kml");
		Document doc = docBuilder.parse(url.toString());
		SpatialTests iut = new SpatialTests();
		iut.setTestSubject(doc);
		iut.verifyTextureFileAliasInModel();
