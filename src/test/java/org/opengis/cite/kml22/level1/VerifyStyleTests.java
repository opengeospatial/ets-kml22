...
/**
 * Verifies the behavior of the test class StyleTests. Test stubs replace
...
public class VerifyStyleTests {
    ...
    @Test
    public void checkStyleTargetAsSameDocFragment_NoErrors()
    ...
    public void checkStyleTargetAsHttpRef_NoErrors() throws SAXException,
            IOException {
        Document doc = docBuilder
                .parse("https://developers.google.com/kml/documentation/KML_Samples.kml");
        when(suite.getAttribute(SUBJ)).thenReturn(doc);
        assertNoErrors(doc);
    }

    @Test
    public void checkStyleTargetAsFileRef_NoErrors() throws SAXException,
            IOException {
        URL url = this.getClass().getResource(
                "/kml/styles/styleUrl-fileRef.xml");
    ...
    @Test
    public void checkStyleTargetAsFileRef_OneError() throws SAXException,
            IOException {
        URL url = this.getClass().getResource(
                "/kml/styles/styleUrl-fileRef.xml");
        Document doc = docBuilder.parse(url.toString());
        ...
        Node styleUrl = doc.getDocumentElement()
                .getElementsByTagNameNS(Namespaces.KML22, "styleUrl").item(1);
    ...
    private void assertNoErrors(Document doc) {
        ...
        iut.setTestSubject(doc);
        Node styleUrl = doc.getDocumentElement()
                .getElementsByTagNameNS(Namespaces.KML22, "styleUrl").item(0);
        iut.checkStyleTarget(styleUrl, this.errHandler);
        assertFalse("Expected no errors\n" + this.errHandler.toString(),
                this.errHandler.errorsDetected());
    }
