...
public class VerifyRecommendations {
	...
	@Test
	public void httpQueryIsMissingSupportedParam() throws SAXException, IOException {
		thrown.expect(AssertionError.class);
		thrown.expectMessage("kml:httpQuery element does not contain a supported parameter");
		URL url = this.getClass().getResource("/kml/links/Link-BadHttpQuery.xml");
		Document doc = docBuilder.parse(url.toString());
		Recommendations iut = new Recommendations();
		iut.setTestSubject(doc);
		iut.verifyHttpQuery();
	...
	public void imagePyramidWithoutLinkHref() throws SAXException, IOException {
		...
		URL url = this.getClass().getResource("/kml/features/PhotoOverlay-PyramidWithoutHref.xml");
		Document doc = docBuilder.parse(url.toString());
		Recommendations iut = new Recommendations();
		iut.setTestSubject(doc);
	...
	@Test
	public void overlayLinkParamsWithoutImagePyramid() throws SAXException, IOException {
		thrown.expect(AssertionError.class);
		thrown.expectMessage("contains kml:Icon/kml:href with parameters but kml:ImagePyramid not found");
		URL url = this.getClass().getResource("/kml/features/PhotoOverlay-HrefParamsWithoutImagePyramid.xml");
		Document doc = docBuilder.parse(url.toString());
		Recommendations iut = new Recommendations();
		iut.setTestSubject(doc);
		iut.verifyImagePyramidInPhotoOverlay();
